...
def metric_tensor(
    ...
    **Example**
    ...
    if approx is None:
        return _metric_tensor_hadamard(
...
def _contract_metric_tensor_with_cjac(mt, cjac):
    ...
    if isinstance(cjac, tuple):
        ...
        metric_tensors = tuple(
            qml.math.tensordot(
                c, qml.math.tensordot(mt, c, axes=[[-1], [0]]), axes=[[0], [0]]
            )
            for c in cjac
            if c is not None
        )
    ...
    is_square = cjac.shape == (1,) or (
        cjac.ndim == 2 and cjac.shape[0] == cjac.shape[1]
    )
    ...
    mt = qml.math.tensordot(
        cjac, qml.math.tensordot(mt, cjac, axes=[[-1], [0]]), axes=[[0], [0]]
    )
    ...
    try:
        ...
        if allow_nonunitary:
            ...
            return qml.ControlledQubitUnitary(
                mat, control_wires=aux_wire, wires=op.wires
            )
...
def _metric_tensor_hadamard(
    ...
    graph = tape.graph
    par_idx_to_trainable_idx = {
        idx: i for i, idx in enumerate(sorted(tape.trainable_params))
    }
    layers = []
    ...
    for idx_i, layer_i in enumerate(layers):
        ...
        for layer_j in layers[idx_i + 1 :]:
            _tapes, _ids = _get_first_term_tapes(
                layer_i, layer_j, allow_nonunitary, aux_wire
            )
    ...
    def processing_fn(results):
        ...
        if ids:
            ...
            first_term = qml.math.scatter_element_add(
                first_term, list(zip(*ids)), off_diag_res
            )
            first_term = qml.math.scatter_element_add(
                first_term, list(zip(*inv_ids)), off_diag_res
            )
        ...
        for i, (layer_i, obs_i) in enumerate(zip(layer_ids, obs_ids)):
            if layer_i is not None and obs_i is not None:
                ...
                expvals = qml.math.scatter_element_add(
                    expvals, (i,), qml.math.dot(l, p)
                )
        ...
        scale = qml.math.convert_like(
            qml.math.tensordot(extended_coeffs_list, extended_coeffs_list, axes=0),
            results[0],
        )
...
def _get_aux_wire(aux_wire, tape, device_wires):
    ...
    if aux_wire is not None:
        ...
        raise qml.wires.WireError(
            "The requested aux_wire does not exist on the used device."
        )

    if device_wires is not None:
        unused_wires = qml.wires.Wires(
            device_wires.toset().difference(tape.wires.toset())
        )
        if not unused_wires:
